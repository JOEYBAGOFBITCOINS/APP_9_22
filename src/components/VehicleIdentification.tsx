...
export const VehicleIdentification: React.FC<VehicleIdentificationProps> = ({
  ...
  const handleVinDetected = (detectedVin: string) => {
    setVin(detectedVin);
    toast.success(`VIN detected: ${detectedVin}`);
  };
  ...
  if (showCamera) {
    return (
      <CameraCapture
        isOpen={showCamera}
        onCapture={(photoUrl) => {
          handleVinPhotoCapture(photoUrl);
        }}
        onClose={() => setShowCamera(false)}
        title="Capture VIN Photo"
        subtitle="Take a clear photo of the VIN label"
      />
    );
  }

  return (
    <div className="fixed inset-0 w-full h-full overflow-y-auto bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 p-4 overscroll-behavior-contain">
